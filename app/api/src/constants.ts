...
assert(
  process.env.CLOUDFRONT_S3_PATTERN,
  'CLOUDFRONT_S3_PATTERN must be present'
);
assert(
...
export const s3BaseUrl = `https://${process.env.CLOUDFRONT_DOMAIN_NAME}/${process.env.CLOUDFRONT_S3_PATTERN}`;
export const cloudfrontDomainName = process.env.CLOUDFRONT_DOMAIN_NAME;
export const cloudfrontPrivateKey = process.env.CLOUDFRONT_PRIVATE_KEY_PATH
  ? readFileSync(process.env.CLOUDFRONT_PRIVATE_KEY_PATH, 'utf-8')
